...
LOG_PATH = os.path.join(ROOT, "runtime_proof_server_medium_rerun.log")
OUT_PATH = os.path.join(ROOT, "runtime_toggle_proof_result_medium_rerun.json")
...
def _stream_sse(scan_id: str):
    ...
    # Always send Authorization header; some deployments do not accept token in query.
...
def _run_case(label: str, control_external_tools: bool):
    ...
    return {
        ...
        "log_excerpt": "\n".join(log_chunk.splitlines()[:220]),
    }


on_case = _run_case("ON_medium", True)
off_case = _run_case("OFF_medium", False)
...
print(json.dumps({
    "artifact": OUT_PATH,
    "on": {
        "scan_id": on_case["scan_id"],
        "status": on_case["status"],
        "score": on_case["score"],
        "phase_count": on_case["sse"]["phase_count"],
        "phase_5b_cli": on_case["log_evidence"]["phase_5b_cli"],
        "nuclei_executing": on_case["log_evidence"]["nuclei_executing"],
        "phase_5b_skipped": on_case["log_evidence"]["phase_5b_skipped"],
        "phase_5d_skipped": on_case["log_evidence"]["phase_5d_skipped"],
    },
    "off": {
        "scan_id": off_case["scan_id"],
        "status": off_case["status"],
        "score": off_case["score"],
        "phase_count": off_case["sse"]["phase_count"],
        "phase_5b_cli": off_case["log_evidence"]["phase_5b_cli"],
        "nuclei_executing": off_case["log_evidence"]["nuclei_executing"],
        "phase_5b_skipped": off_case["log_evidence"]["phase_5b_skipped"],
        "phase_5d_skipped": off_case["log_evidence"]["phase_5d_skipped"],
        "external_tools_disabled": off_case["log_evidence"]["external_tools_disabled"],
    }
}, indent=2))
